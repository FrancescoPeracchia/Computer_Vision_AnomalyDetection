...
def train_model(csv_file, data_dir, ep,lr):
    ...
    #robe che servono e path
    file_number = get_a_random_pair #(sistemare era un random tra tot numeri)
    image_path = os.getcwd()+"\\Figures" #path del dataset
    Models_path = os.getcwd()+"\\Re_Traind_Models" #qui va messa la funzione del modello 
    
    plot_Name = image_path+"\\VGG16_"+str(file_number)+".png"
    plt.savefig(plot_Name , transparent =True , bbox_incehs="tight" , pad_inches = 2 , dpi = 50)#forse non serve
    #salviamo il modello
    model.save(Model_Name)
    model.save_weights('visiope_bestmodel.hdf5')
